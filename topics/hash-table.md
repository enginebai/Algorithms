...
_Sets_ maintain a collection of items in an _intrinsic_ order based on a **unique key**, each item `x` has key `x.key`. Sets are generattion of `set` and `dictionary`.
...
A dynamic set that supports the _dictionary_ operations: `insert(x)`, `delete(k)` and `get(k)`, and there are some to fulfill this:

- **Direct access address**: it's effificent to `get(k)`, however, it takes lots of space.
- **Hashing**: We compute the array index from key.
- **Chaining**: It's a way to handle _collisions_ for hashing.
...
The disadvantage of direct access table is obvious, the actual keys might be small relative to `U`. To fix this, we can use _hash function_ `h(key)` to reduce space to `Θ(|K|)` and it still takes only `O(1)` for searching.

The hash function `h(key)` calculates the _hash value_ of key `k`, maps the universe `U` into the slots of a _hash table_ `T[i]`.

For this mechanism, we will face a problem: _collision_, that is, the two different keys may hash to the same slot.
...
In _chaining_, we store all values that have the same hash value in a [linked list](../topics/linked-list.md)
...
Given a hash table `T` with `m` slots that stores `n` elements, _load factor_ `n / m` (i.e. `items size / table size`) defines the average number of elements stored in the chain of one slot.

The worst case running time is `Θ(n)` when all elements storing to the same slot.

The average running time depends the list length of a slot, and the list length depends on **how well the hash function** distributes key among the slots, we assume that any element is **equally likely** to hash into any slots, independently of where any other element hash hashed to, it called _simple uniform hashing_, the list length will be `n / m` mentioned above on average, so successful or unsuccessful search takes `Θ(1 + n / m)` time. (`Θ(1)` for hash function calculation).
...
We store all element directly in the hash table itself and we keep looking for next empty slot to insert the key until we find it or the table is full. (called _probe_)
...
// Custom comparator
// Descending order - simpler alternatives
val customMap1 = TreeMap<Int, Int>(reverseOrder()) // Simplest: using reverseOrder()
val customMap2 = TreeMap<Int, Int> { a, b -> b.compareTo(a) } // Manual: reverse comparison
val customMap3 = TreeMap<String, Int>(compareBy { it.length }) // Sort by string length
// Tie-breaker: sort by string length first, then alphabetically
val tieBreakerMap = TreeMap<String, Int>(
    compareBy<String> { it.length }.thenBy { it }
)

// Sorted order operations (unique to TreeMap)
val firstKey: Int? = map.firstKey() // Smallest key
val lastKey: Int? = map.lastKey() // Largest key
val firstEntry: Map.Entry<Int, Int>? = map.firstEntry() // Entry with smallest key
val lastEntry: Map.Entry<Int, Int>? = map.lastEntry() // Entry with largest key

// Navigable operations
// map.xxxKey() or map.xxxEntry()
val lowerKey: Int? = map.lowerKey(123) // Greatest key < 123
val higherKey: Int? = map.higherKey(123) // Least key > 123
val floorKey: Int? = map.floorKey(123) // Greatest key <= 123
val ceilingKey: Int? = map.ceilingKey(123) // Least key >= 123

// Iteration (maintains sorted order by key)
for (key in map.keys) { ... }
for (value in map.values) { ... }
for ((key, value) in map) { ... }
for (entry in map.entries) { ... }
...
// Descending order: O(1) in TC and SC
...
| ----------------- | ------------------------- | --------------------------------------------------- | ---------------------- |
| **Insert/Update** | `map[key] = value`        | Add or update entry                                 | O(log n)               |
| **Get**           | `map[key]`                | Get value, null if absent                           | O(log n)               |
| **Remove**        | `map.remove(key)`         | Remove entry                                        | O(log n)               |
| **First/Last**    | `firstKey()`, `lastKey()` | Min/max key                                         | O(log n)               |
| **Lower**         | `lowerKey(key)`           | Largest key **<** given key                         | O(log n)               |
| **Floor**         | `floorKey(key)`           | Largest key **≤** given key                         | O(log n)               |
| **Higher**        | `higherKey(key)`          | Smallest key **>** given key                        | O(log n)               |
| **Ceiling**       | `ceilingKey(key)`         | Smallest key **≥** given key                        | O(log n)               |
| **Create View**   | `map.descendingMap()`     | Creates view only for one-time descending iteration | **O(1) for TC and SC** |
