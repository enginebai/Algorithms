...
fun isSubPath(head: ListNode?, root: TreeNode?): Boolean {
    ...
    return contains(head, root) ||      // Search the linked list starting from the root node.
        isSubPath(head, root.left) ||   // Or search the linked list starting from the left and right subtree.
