...
- Initial `minutes = 0` and return `minutes`, and `while (freshCount > 0 && queue.isNotEmpty()) { ... }`
- Initial `minutes = 0` and return `minutes`, and check `if (rotten) minutes++` to ensure there is any fresh orange rotten.
