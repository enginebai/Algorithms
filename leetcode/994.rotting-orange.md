...
fun orangesRotting(grid: Array<IntArray>): Int {
    ...
    // `minutes - 1` because we increment the minutes after the last level
    // Queue:    A -> B -> C -> D
    // Minutes     1    2    3    4
...
We can start from the initial rotten oranges and **enqueue the fresh oranges nearby** to be rotten. We don't enqueue the starting rotten oranges, so we don't have to -1 when returning the minutes.
