...
* If there is no fresh orange, the time should be 0 whether there are rotten oranges or not.
    * No fresh, no rotten `[0,0,0]`: 0
    * No fresh, has rotten `[0,1]`: 0
* If there is fresh orange, then it might be impossible or possible to rot all the fresh oranges.
    * Has fresh, no rotten `[0,2]`: -1
    * Has fresh, has rotten, but not reachable `[1,0,2]`: -1
* There are multiple rotten oranges, and some fresh oranges will be rotten by multiple rotten oranges, and some fresh oranges will be rotten by one rotten orange. `[2,1,1,1,2,1]`: 2
