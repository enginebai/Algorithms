...
## Key Insights
The problem is asking for the time that signal reaches all node with directed, positive weighted edges (travel times). In graph theory, we're looking for the "maximum of all shortest paths from a single source". Single-source shortest path with positive weights = Dijkstra.

Q: Why does a greedy (Dijkstra) approach work here? Because that time is monotonic and all edge weights are positive, it's impossible to find a "faster" path by traveling through a node that takes longer to reach. When a node popped from the priority queue, it guarantees to be the shortest path to that node.

## Textbook Dijkstra (Distance Array)
### Adjacency List
...
private fun buildGraph(times: Array<IntArray>, n: Int): Array<MutableList<Edge>> {
...
We use *adjacency matrix* (`graph[from][to] = weight`) to build the graph:
...
The implementation relies entirely on the **monotonicity of min-heap**, because there is no negative weights, the very first time a node is popped from the heap, it's mathematically impossible to find a shorter path to it later. Any subsequent path discovered will traverse more edges, only adding more cost since weights are positive. Therefore, the first popped node is the *absolute shortest path*, and any later popped same nodes are stable and can simple be ignored by `is (visited[node]) continue`.
