...
1. Everyone gets the same number of items.
2. Everyone gets only one pile.
3. We can break down the same pile into several sub-piles, but we can't merge the piles.

Find the max number of items.

## Derivation
- Abstraction: For some number `X`, we check if we can allocate `X` to `k` children and we find the maximum `X`.

- Brute force: We start `X` from 1, 2, 3, …, try to find the maximum `X` that we can allocate or the minimum `X` that we can’t.

- Bottleneck analysis: If we know `X` is feasible, then smaller `X` is also feasible, then it’s a waste to check any smaller value < `X`. If `X` is infeasible, then larger value > `X` is also infeasible and also a waste to check.

- Pattern recognition: It’s a monotonicity from our observation of bottleneck analysis. And we have a fixed search range:

   - Lower bound: `1`, everyone only gets one items.
   - Upper bound: `max(piles)`, one person can get the largest pile.

→ A fixed search range + monotonicity = Binary Search on Answer
...
* Each child can take at most one pile of candies and some piles of candies may go unused. `[8] = 2 / 2` to two children (`2` are unsed), it can't be `3 / 3 / 2` to 3 children.
