...
A _topological sort_ of a directed acyclic graph (DAG) is a _linear ordering_ of all vertices such that `x -> y` which `x` is the ancestor of `y`. Topological sorting helps process nodes in dependency order. We can use it to find all ancestors of a node.
