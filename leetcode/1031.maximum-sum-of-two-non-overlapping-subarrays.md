...
> How can we deal with the order of the two subarrays?
Since subarray **can appear in either ordre**, the maximum sum can change, it could be `[..., first max, ..., second max, ...]` or `[..., second max, ..., first max, ...]`, it could be `firstLen` before `secondLen` or `secondLen` before `firstLen`, so we must try both permutations and return the maximum of them: 
