...
> In [227. Basic Calculator II](../leetcode/227.basic-calculator-ii.md), precedence was dictated by the operator `*` and `/`. Here it's dictated purely by **scope** (parentheses). Because the parentheses can be nested infinitely, the underlying structure is inherently recursivie.
...
* The operator `+/-`: We need to calculate the result based on the operator.
    - Here please note that we apply the same login in [227. Basic Calculator II](../leetcode/227.basic-calculator-ii.md) to sum up the previous result when we encounter the new operator. 
```js
              -,   5   +,   8   -,   (,   4   +,   2,   )
num = 0       0    5   0    8   8
sign = 1     -1   -1   1    1  -1
...
fun calculate(s: String): Int {
    ...
    for (i in 0 until s.length) {
        if (s[i].isDigit()) {
        ...
        else if (s[i] == '(') { // We enter a new scope, preserve the current result + sign before the parenthese into stack
            // XYZ +/- (
            // ^^^ ^^^
            stack.push(result)  // Push XYZ
            stack.push(sign)    // Push +/-
            
            // Reset all state and use them to calculate the value inside parenthesis
            result = 0
            number = 0
            sign = 1
        } else if (s[i] == ')') {
            ...
            val signBeforeParenthsis = stack.pop() // It will be the sign before the parenthese, i.e. 1
            val previousResult = stack.pop() // It's previous result calculated before the parenthese, i.e. 10

            result = previousResult + signBeforeParenthsis * result
    ...
    // Remember to calculate the remaining number, from above example, it ..)-5
