...
Based on the two observations we can apply binary search based on the two observations: We start at top-right corner (or bottom-left), the corners allow us to know where one direction decrease and the other increases, facilitating search space reduction.

We compare the current number with `target`, return true if we found it, otherwise, we go left if `target < middle`, or go down if `middle < target`.
